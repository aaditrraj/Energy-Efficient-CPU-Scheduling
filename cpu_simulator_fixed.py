...
# Overhead penalties
CS_OVERHEAD_ENERGY = 0.01
MIGRATION_ENERGY   = 0.03
CS_OVERHEAD_TIME   = 0.002
...
ARCHITECTURE = "smp" # "smp" or "biglittle"
...
class Task:
    ...
    def clone(self):
        t = Task(self.tid, self.wcet, self.arrival, self.deadline, self.priority)
        t.remaining = self.remaining
        t.start_time = self.start_time
        t.finish_time = self.finish_time
        t.last_core = self.last_core
...
# ============================================================
def dvfs_power(freq_frac, active_indices, current_temp=AMBIENT_TEMP):
    """Total power = dynamic + static across clusters."""
    voltage = 1.0
    ...
    leakage_mult = 1.0 + 0.015 * max(0, current_temp - 25.0)
    total_power = 0.0
    ...
    for i in range(MAX_CORES):
        ...
        if ARCHITECTURE == "biglittle":
            if i < 2: # LITTLE cores
                c_dyn = 0.5
                p_stat = 0.03
                actual_freq = min(freq_frac, 0.6)
                actual_v = 1.0
                for f, v in DVFS_LEVELS:
                    if abs(f - actual_freq) < 0.01:
                        actual_v = v
                        break
                if is_active: total_power += c_dyn * (actual_v ** 2) * actual_freq
                total_power += p_stat * leakage_mult
            else: # big cores
                c_dyn = 1.8
                p_stat = 0.15
                if is_active: total_power += c_dyn * (voltage ** 2) * freq_frac
                total_power += p_stat * leakage_mult
        else:
            if is_active: total_power += c_dyn * (voltage ** 2) * freq_frac
            total_power += p_stat * leakage_mult
...
def cycles_per_second(freq_frac, core_idx):
    perf = PERF_CONSTANT
    if ARCHITECTURE == "biglittle" and core_idx < 2:
        perf *= 0.5 # LITTLE cores are slower
    return freq_frac * perf * 1000.0
...
class EATSStepper:
    ...
    Enhanced with Cluster-based big.LITTLE and CS/Migration penalties.
    """
    NAME = "EATS (Proposed)"

    def __init__(self, tasks):
        self.tasks       = [t.clone() for t in tasks]
        self.now         = 0.0
        ...
        self.last_running = [None] * MAX_CORES
    ...
    def _pick_config(self, ready):
        ...
        for f, _v in DVFS_LEVELS:
            ...
            for n in range(1, MAX_CORES + 1):
                ...
                total_rate = 0
                for i in range(n):
                    total_rate += f * 0.5 if (ARCHITECTURE == "biglittle" and i < 2) else f
                
                for t in ready:
                    avg_rate = total_rate / n if n > 0 else 1e-9
                    if self.now + t.remaining / avg_rate > t.deadline + 1e-9:
                        if t.priority in ("irq", "critical"):
                            feasible = False
                            break
                if feasible:
                    pw = dvfs_power(f, list(range(n)), self.temperature)
    ...
    def step(self, dt=DT):
        ...
        ready = sorted(self.runnable(), key=lambda t: (
            {"irq": -1, "critical": 0, "normal": 1, "background": 2}.get(t.priority, 1),
            t.deadline,
        ))
        for t in ready:
            if t.start_time is None: t.wait_time += dt

        self.freq, self.cores = self._pick_config(ready)
        running_tid = None
        work_done   = 0.0
        active_indices = list(range(self.cores))
        penalty_energy = 0.0
        new_running = [None] * MAX_CORES

        for i in range(self.cores):
            if i >= len(ready): break
            t = ready[i]
            
            if self.last_running[i] != t.tid:
                penalty_energy += CS_OVERHEAD_ENERGY
                if t.last_core is not None and t.last_core != i:
                    penalty_energy += MIGRATION_ENERGY
            
            rate_sec = self.freq * 0.5 if (ARCHITECTURE == "biglittle" and i < 2) else self.freq
            do = min(t.remaining, rate_sec * dt)
            if do <= 0 and t.remaining > 0:
        ...
        self.last_running = new_running
        pw = dvfs_power(self.freq, active_indices, self.temperature)
        self.energy      += (pw * dt) + penalty_energy
        self.temperature  = thermal_step(self.temperature, pw, dt)
        self.now         += dt
        
        total_cap = 0
        for i in range(self.cores):
            total_cap += self.freq * 0.5 * dt if (ARCHITECTURE == "biglittle" and i < 2) else self.freq * dt
        util = min(work_done / total_cap if total_cap > 0 else 0, 1.0)
...
class PerformanceFirstStepper:
    """Always max frequency + all cores."""
    ...
    def __init__(self, tasks):
        ...
        self.energy      = 0.0
        self.temperature = AMBIENT_TEMP
        self.history     = {k: [] for k in ("t", "energy", "freq", "cores", "util", "temp", "power", "running_task")}
        self.last_running = [None] * MAX_CORES

    def step(self, dt=DT):
        if all(t.is_done() for t in self.tasks) or self.now >= SIM_DURATION: return False
        ready = sorted([t for t in self.tasks if t.is_ready(self.now)], key=lambda t: t.deadline)
        for t in ready:
            if t.start_time is None: t.wait_time += dt

        work_done, running_tid, penalty_energy = 0.0, None, 0.0
        new_running = [None] * MAX_CORES
        for i in range(MAX_CORES):
            if i >= len(ready): break
            t = ready[i]
            if self.last_running[i] != t.tid:
                penalty_energy += CS_OVERHEAD_ENERGY
                if t.last_core is not None and t.last_core != i: penalty_energy += MIGRATION_ENERGY
            rate_sec = self.freq * 0.5 if (ARCHITECTURE == "biglittle" and i < 2) else self.freq
            do = min(t.remaining, rate_sec * dt)
            if t.start_time is None and do > 0: t.start_time = self.now
            t.remaining -= do
            work_done += do
            t.last_core, new_running[i], running_tid = i, t.tid, t.tid
            if t.is_done(): t.finish_time = self.now + dt

        self.last_running = new_running
        pw = dvfs_power(self.freq, list(range(MAX_CORES)), self.temperature)
        self.energy += (pw * dt) + penalty_energy
        self.temperature = thermal_step(self.temperature, pw, dt)
        self.now += dt
        total_cap = 0
        for i in range(MAX_CORES): total_cap += self.freq * 0.5 * dt if (ARCHITECTURE == "biglittle" and i < 2) else self.freq * dt
        util = min(work_done / total_cap if total_cap > 0 else 0, 1.0)
        h = self.history
        h["t"].append(self.now); h["energy"].append(self.energy); h["freq"].append(self.freq)
        h["cores"].append(self.cores); h["util"].append(util); h["temp"].append(self.temperature)
        h["power"].append(pw); h["running_task"].append(running_tid)
        return True

class RoundRobinStepper:
    """Fixed mid-level frequency, all cores."""
    NAME = "Round-Robin"
    ...
    def step(self, dt=DT):
        if all(t.is_done() for t in self.tasks) or self.now >= SIM_DURATION: return False
        ready = sorted([t for t in self.tasks if t.is_ready(self.now)], key=lambda t: t.tid)
        for t in ready:
            if t.start_time is None: t.wait_time += dt
...
class SJFStepper:
    ...
    def __init__(self, tasks):
        self.tasks = [t.clone() for t in tasks]
        self.now, self.energy, self.temperature = 0.0, 0.0, AMBIENT_TEMP
        self.freq, self.cores = 1.0, MAX_CORES
        self.history = {k: [] for k in ("t", "energy", "freq", "cores", "util", "temp", "power", "running_task")}
        self.last_running = [None] * MAX_CORES
    def step(self, dt=DT):
        if all(t.is_done() for t in self.tasks) or self.now >= SIM_DURATION: return False
        ready = sorted([t for t in self.tasks if t.is_ready(self.now)], key=lambda t: t.remaining)
        for t in ready:
            if t.start_time is None: t.wait_time += dt
        work_done, running_tid, penalty_energy = 0.0, None, 0.0
        new_running = [None] * MAX_CORES
...
class FCFSStepper:
    ...
    def step(self, dt=DT):
        if all(t.is_done() for t in self.tasks) or self.now >= SIM_DURATION: return False
        ready = sorted([t for t in self.tasks if t.is_ready(self.now)], key=lambda t: (t.arrival, t.tid))
        for t in ready:
            if t.start_time is None: t.wait_time += dt
        work_done, running_tid, penalty_energy = 0.0, None, 0.0
        new_running = [None] * MAX_CORES
...
def _calc_missed(stepper):
    return sum(
        ...
        if t.deadline is not None and (
            (t.finish_time is not None and t.finish_time > t.deadline + 1e-6)
            or (not t.is_done() and stepper.now > t.deadline + 1e-6)
        )
    )

def _calc_avg_wait(stepper):
    waits = [t.wait_time for t in stepper.tasks if t.start_time is not None]
...
class CPUSimApp:
    def __init__(self, master):
        ...
        ttk.Combobox(ctrl, textvariable=self.sched_var,
                     values=["EATS (Proposed)", "Performance-First", "Round-Robin", "Shortest Job First (SJF)", "First Come First Serve (FCFS)"],
                     width=25, state="readonly").grid(row=0, column=1, sticky=tk.W)
        ...
        ttk.Label(ctrl, text="CPU Arch:").grid(row=0, column=6, sticky=tk.W, padx=(12, 4))
        self.arch_var = tk.StringVar(value="smp")
        ttk.Combobox(ctrl, textvariable=self.arch_var, values=["smp", "biglittle"], width=10, state="readonly").grid(row=0, column=7, sticky=tk.W)

        ttk.Label(ctrl, text="Speed:").grid(row=0, column=8, sticky=tk.W, padx=(12, 4))
        self.speed_var = tk.DoubleVar(value=1.0)
        ttk.Scale(ctrl, variable=self.speed_var, from_=0.25, to=4.0,
                  orient=tk.HORIZONTAL, length=120).grid(row=0, column=9, sticky=tk.W)
        ...
        self.start_btn.grid(  row=0, column=10,  padx=(12, 4))
        self.pause_btn.grid(  row=0, column=11,  padx=4)
        self.reset_btn.grid(  row=0, column=12, padx=4)
        self.compare_btn.grid(row=0, column=13, padx=4)
        self.batch_btn.grid(  row=0, column=14, padx=4)
    ...
    def start(self):
        ...
        Cls = {"EATS (Proposed)": EATSStepper,
               ...
               "Round-Robin": RoundRobinStepper,
               "Shortest Job First (SJF)": SJFStepper,
               "First Come First Serve (FCFS)": FCFSStepper}[sched]
    ...
    def compare_all(self):
        ...
        def worker():
            ...
            res = {}
            for Cls in (EATSStepper, PerformanceFirstStepper, RoundRobinStepper, SJFStepper, FCFSStepper):
    ...
    def batch_run(self):
        ...
        def worker():
            ...
            for Cls in (EATSStepper, PerformanceFirstStepper, RoundRobinStepper, SJFStepper, FCFSStepper):
                for seed in range(1, n_seeds + 1):
                    ...
                    rows.append({
                        ...
                        "avg_wait_s":       round(_calc_avg_wait(s), 4),
                    })
                    # per-run graph
                    try:
            ...
            try:
                ...
                names_u = ["EATS (Proposed)", "Performance-First", "Round-Robin", "Shortest Job First (SJF)", "First Come First Serve (FCFS)"]
