...
Simulates and compares five CPU scheduling strategies — EATS, Performance-First, Round-Robin, Shortest Job First (SJF), and First Come First Serve (FCFS) — with real-time animated visualizations of energy consumption, thermal behavior, DVFS scaling, and core utilization.
...
- **Context Switching & Migration Overhead** — models realistic energy and time penalties for task switching and moving tasks between cores
- **Advanced big.LITTLE Model** — cluster-based power modeling with distinct power/performance profiles for LITTLE and big cores
